...
Usage:
  ...
  ./scripts/release.sh package-binary
...
parse_args() {
  if [[ $# -gt 0 ]]; then
    case "$1" in
      release|prepare|publish|package-binary)
        MODE="$1"
        shift
        ;;
      --help)
  ...
  case "$MODE" in
    ...
    publish)
      ...
      ;;
    package-binary)
      [[ $# -eq 0 ]] || fail "package-binary does not accept additional arguments"
      ;;
    *)
